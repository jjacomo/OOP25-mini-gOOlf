...
public final class WallObstacle extends AbstractObstacle {
    ...
    private Vector2D computeNormalForBoundaryCase(final Vector2D ballPos) {
		...
		if (Math.abs(dLeft - minDist) <= tolerance)   accX -= 1;
		if (Math.abs(dRight - minDist) <= tolerance)  accX += 1;
		if (Math.abs(dTop - minDist) <= tolerance)    accY -= 1;
		if (Math.abs(dBottom - minDist) <= tolerance) accY += 1;
